...
@section('title', 'Edit Event & Promo')
    ...
    <div class="container">
        ...
        <div class="card card-form-event">
            <div class="card-form-event-header">
                <h2 class="fw-bold mb-1">Edit Event &amp; Promo</h2>
                <p class="mb-0 opacity-75">Perbarui detail "{{ $event->judul }}"</p>
            </div>
            ...
            <div class="card-form-event-body">
                ...
                <form action="{{ route('event.update', $event->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="judul" id="judul" class="form-control" value="{{ old('judul', $event->judul) }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="promo" class="form-label fw-semibold">Label Promo</label>
                        <input type="text" name="promo" id="promo" class="form-control" value="{{ old('promo', $event->promo) }}" required>
                    </div>

                    <div class="mb-3">
                        <label for="deskripsi" class="form-label fw-semibold">Deskripsi</label>
                        <textarea name="deskripsi" id="deskripsi" class="form-control" rows="4">{{ old('deskripsi', $event->deskripsi) }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label for="gambar" class="form-label fw-semibold">Gambar</label>
                        @if (!empty($event->gambar))
                            <img src="{{ asset('storage/' . $event->gambar) }}" alt="{{ $event->judul }}" class="gambar-preview-lama d-block">
                        @endif
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="tanggal_mulai" id="tanggal_mulai" class="form-control" value="{{ old('tanggal_mulai', $event->tanggal_mulai) }}">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="tanggal_selesai" class="form-label fw-semibold">Tanggal Selesai</label>
                            <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="form-control" value="{{ old('tanggal_selesai', $event->tanggal_selesai) }}">
                        </div>
                    </div>

                    <div class="mb-4">
                        <label for="urutan" class="form-label fw-semibold">Urutan Tampil</label>
                        <input type="number" name="urutan" id="urutan" class="form-control" min="0" value="{{ old('urutan', $event->urutan) }}">
                    </div>
